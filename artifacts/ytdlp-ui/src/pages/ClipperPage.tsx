...
// YouTube sometimes blocks server IPs with a "Sign in to confirm you're not a
// bot" wall. Uploading a cookies.txt exported from a signed-in browser lets
// the server pass that check and keeps the fast clip path working.
...
function CookiesPanel() {
  ...
  const loadStatus = () => {
    fetch(`${API}/ytdlp/cookies/status`, { credentials: 'include' })
  ...
  const save = async (text: string) => {
    ...
    try {
      ...
      setMsg({ kind: 'ok', text: `Cookies saved — ${data.youtubeCookieCount} YouTube cookies detected.` });
      setCookiesText('');
  ...
  const remove = async () => {
    ...
    try {
      ...
      setMsg({ kind: 'err', text: 'Could not remove cookies.' });
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-3">
      ...
      {open && (
        <div className="mt-4 bg-[#161616] border border-white/8 rounded-2xl p-5 space-y-4 text-left">
          <p className="text-white/60 text-sm leading-relaxed">
            YouTube sometimes blocks our server with a &ldquo;Sign in to confirm you&rsquo;re not a bot&rdquo;
            check. Uploading a <span className="text-white/85 font-semibold">cookies.txt</span> exported from
            your browser keeps YouTube clipping fast and reliable.
          </p>
          <ol className="text-white/50 text-xs leading-relaxed list-decimal pl-4 space-y-1">
            <li>Install a cookies exporter extension (e.g. &ldquo;Get cookies.txt LOCALLY&rdquo; for Chrome/Firefox).</li>
            <li>Open <span className="text-white/75">youtube.com</span> while signed in, click the extension, and export in Netscape format.</li>
            <li>Upload or paste the file below.</li>
          </ol>
          <p className="text-amber-300/70 text-xs leading-relaxed">
            ⚠️ Safety note: cookies act like a login for the account they came from. Prefer a throwaway/secondary
          ...
          {status?.configured && (
            <div className="flex items-center justify-between bg-[#1e1e1e] border border-white/10 rounded-xl px-3 py-2.5">
              <span className="text-white/70 text-xs font-semibold">
                {status.source === 'env'
                  ? 'Cookies configured by the server operator.'
                  : `Cookies active — ${status.youtubeCookieCount} YouTube cookies.`}
              </span>
              {status.source === 'uploaded' && (
                <button
                  ...
                  onClick={remove}
                  disabled={busy}
                  className="text-red-400/80 hover:text-red-300 text-xs font-bold transition-colors disabled:opacity-40"
                >
                  Remove
                </button>
              )}
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-[#1e1e1e] text-white/80 text-xs font-mono border border-white/10 rounded-xl px-3 py-2.5 outline-none focus:border-[#D1FE17]/50 transition-colors resize-y"
          />
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => save(cookiesText)}
              disabled={busy || !cookiesText.trim()}
              className="bg-[#D1FE17] text-black text-xs font-black px-4 py-2 rounded-xl hover:bg-[#c5f010] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={busy}
              className="bg-[#1e1e1e] border border-white/10 text-white/70 hover:text-white text-xs font-bold px-4 py-2 rounded-xl transition-all disabled:opacity-40"
            >
              Upload cookies.txt
            </button>
            <input
          ...
          {msg && (
            <p className={`text-xs font-semibold ${msg.kind === 'ok' ? 'text-[#D1FE17]' : 'text-red-400'}`}>
              {msg.text}
            </p>
          )}
        </div>
