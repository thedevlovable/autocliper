...
// Returns whether the user has connected their own Buffer account via OAuth.
router.get("/user/buffer/status", requireUser, async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.json({
      ...
      // also expose whether admin fallback is available
      adminConfigured: isBufferConfigured(),
    });
  } catch { res.json({ connected: false, adminConfigured: false }); }
});

// ── GET /user/buffer/channels ──────────────────────────────────────────────────
// Returns channels for the signed-in user.
// Priority: (1) user's own Buffer token → own channels; (2) admin fallback.
router.get("/user/buffer/channels", requireUser, async (req, res): Promise<void> => {
  const userId = req.currentUser!.id;
  try {
    // Check for user's own Buffer OAuth token
    ...
    if (tokenRows.length > 0) {
      // User has their own Buffer — serve their channels
      const { rows } = await requireDb().query<{
        id: string; service: string; name: string; enabled: boolean;
      }>(
        `SELECT channel_id AS id, service, name, enabled
         FROM user_buffer_own_channels WHERE user_id = $1 ORDER BY service, name`,
        [userId],
      );
...
// Toggle a channel on/off for the signed-in user (own OR admin-shared channels).
router.patch("/user/buffer/channels/:channelId", requireUser, async (req, res): Promise<void> => {
  ...
  try {
    // Check if it belongs to user's own Buffer account
    const { rows: own } = await requireDb().query(
      `SELECT 1 FROM user_buffer_own_channels WHERE user_id = $1 AND channel_id = $2`,
      [userId, channelId],
    );
    if (own.length > 0) {
